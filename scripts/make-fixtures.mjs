...
const CHAPTERS = [
  {
    ...
    paras: [
      'في هذا الكتاب التجريبي نختبر قدرة القارئ على عرض النصوص العربية بشكل صحيح من اليمين إلى اليسار.',
      'القراءة رحلة ممتعة تنقلنا إلى عوالم مختلفة، وتفتح أمامنا آفاقاً من المعرفة والثقافة لا حدود لها.',
      'جرّب تحديد هذا النص لتمييزه بالألوان أو إضافة تسطير أو كتابة ملاحظة عليه، ثم أغلق الكتاب وافتحه مجدداً لتجد تعليقاتك محفوظة.'
    ]
  },
  {
    title: 'الفصل الثاني: المتن',
    paras: [
      'الكتب الجيدة أصدقاء وفية تبقى معنا مدى الحياة، نعود إليها كلما اشتقنا إليها فنجد فيها ما يريح العقل والقلب.',
      'المكتبة الرقمية تتيح لنا حمل مكتبة كاملة في جيبنا، والتنقل بين الكتب بسهولة وبحث سريع ودقيق.',
      'كلمة للبحث: مكتبة، قراءة، كتاب، معرفة، حكمة.'
    ]
  },
  {
    title: 'الفصل الثالث: الختام',
    paras: [
      'وفي الختام، إن من أحبّ القراءة لم يشعر بالوحدة قط، فبين دفات الكتب تنبض حكايات الأجيال وحكمتها.',
      'شكراً لتجربة تطبيق «مكتبة» — نتمنى لك قراءة ممتعة!'
    ]
  }
]
...
function makeTinyCoverPng() {
  try {
    ...
    function chunk(type, data) {
      ...
      for (let n = 0; n < 256; n++) {
        let c = n
        for (let k = 0; k < 8; k++) c = c & 1 ? 0xedb88332 ^ (c >>> 1) : c >>> 1
        crcTable[n] = c >>> 0
      }
      let crc = 0xffffffff
